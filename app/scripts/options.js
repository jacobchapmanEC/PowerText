...
var ateOptionsModule = (function ($) {
  ...
  function setupUI() {
    ...
    $("#omniboxKeyword").text(
      chrome.i18n.getMessage("KEYWORD_OMNIBAR_TRIGGER")
    );
    ...
    $("#insertMDATag").text(
      ATE_CONST.INSERT_MDA_TAG.replace("\\", "") +
        "attribute_logical_name[,fallback_attribute ...]" +
        ATE_CONST.INSERT_MDA_CLOSE_TAG.replace("\\", "")
    );
    $("#insertDateTag").text(
      ATE_CONST.INSERT_DATE_TAG.replace("\\", "") +
        "dateformat" +
        ATE_CONST.INSERT_DATE_CLOSE_TAG.replace("\\", "")
    );
    $("#insertDateFormat").text(
      ATE_CONST.INSERT_DATE_TAG.replace("\\", "") +
        DATE_MACRO_DEMO_FORMAT +
        ATE_CONST.INSERT_DATE_CLOSE_TAG.replace("\\", "")
    );
    $("#insertDateDemo").text(
      (function () {
    ...
    $("#blacklistButton").html(
      chrome.i18n.getMessage("TITLE_BLACKLIST_BUTTON")
    );
    $("#importPanel .title").text(chrome.i18n.getMessage("TITLE_IMPORT_PANEL"));
    $("#importPanel .description").html(
      chrome.i18n.getMessage("TEXT_IMPORT_PANEL")
    );
    $("#timeoutPanel .title").text(
      chrome.i18n.getMessage("TITLE_TIMEOUT_PANEL")
    );
    $("#timeoutPanel .description").html(
      chrome.i18n.getMessage("TEXT_TIMEOUT_PANEL")
    );
    $("#timeoutDefault").text(DEFAULT_CLEAR_BUFFER_TIMEOUT);
    $("#blacklistPanel .title").text(
      chrome.i18n.getMessage("TITLE_BLACKLIST_PANEL")
    );
    $("#blacklistPanel .description").html(
      chrome.i18n.getMessage("TEXT_BLACKLIST_PANEL")
    );
  ...
  function setupShortcutEditor() {
    ...
    $("#edit").on("blur", "textarea.autotext", function () {
      if (this.value == "") {
  ...
  function setupSettingsPanels() {
    ...
    $("#timeoutSlider").on("mousemove", function () {
      var timeout = $(this).val();
      metadata[ATE_CONST.SHORTCUT_TIMEOUT_KEY] = timeout;
      updateShortcutTimeoutLabel(timeout);
    });
    ...
    $(".panelCancelButton").click(function () {
      togglePanel();
    });
  }
  ...
  function refreshQuotaLabels(shortcuts) {
    ...
    if (!$.isEmptyObject(shortcuts)) {
      ...
      $("#countStorage").text(
        Object.keys(shortcuts).length - Object.keys(metadata).length
      );
    }
  ...
  function processMetadata(data) {
    ...
    if (shortcutTimeout) {
      // If exists, replace metadata
      metadata[ATE_CONST.SHORTCUT_TIMEOUT_KEY] = shortcutTimeout;
    } else {
      // Otherwise, use metadata default value
    ...
    if (
      shortcutVersion &&
      shortcutVersion != metadata[ATE_CONST.SHORTCUT_VERSION_KEY]
    ) {
      ...
      console.error(
        "Extension version:",
        metadata[ATE_CONST.SHORTCUT_VERSION_KEY]
      );
    }
  }
    ...
    $("#edit").fadeOut(ANIMATION_FAST, function () {
      ...
      $(this).fadeIn(ANIMATION_FAST, function () {
        if (!$.isEmptyObject(data)) {
          ...
          $("textarea").autosize();
        } // No shortcuts? Check if first run on this computer
        else {
          chrome.storage.local.get(FIRST_RUN_KEY, function (firstRun) {
            if (chrome.runtime.lastError) {
              // Check for errors
              console.error(chrome.runtime.lastError);
            } else if (!firstRun[FIRST_RUN_KEY]) {
              // First run
              // Flag first run
              ...
              addRow(
                "hbd",
                "Hey! Just wanted to wish you a happy birthday; hope you had a good one!"
              );
              ...
              // Set textarea height to fit content and resize as user types
              $("textarea").autosize();
            } // First run already happened, why no shortcuts??
            else {
              ...
              getLocalBackup(function (data) {
                // Check local backup
                if (!data || $.isEmptyObject(data)) {
                  // No local backup
                  getEmergencyBackup(function (data) {
                    // Check emergency
                    if (!$.isEmptyObject(data)) {
                      // Has backup
        ...
        var refreshTimeInMilliseconds =
          new Date().getTime() - refreshStartTime.getTime();
  ...
  function removeRow() {
    var $this = $(this);
    showModalPopup(
      chrome.i18n.getMessage("MESSAGE_REMOVE_ROW_POPUP") +
        chrome.i18n.getMessage("PROMPT_CONTINUE"),
      function (response) {
        if (response) {
  ...
  function addRow(shortcut, autotext, append) {
    if ($("tbody > tr").length >= adjustedCountQuota) {
      ...
      showCrouton(
        chrome.i18n.getMessage("ERROR_OVER_ITEM_QUOTA") +
          " Max # Items: " +
          adjustedCountQuota,
        "red"
      );
  ...
  function validateRow($input, completionBlock) {
    ...
    if (itemSize >= QUOTA_ITEM_STORAGE) {
      ...
      errors.autotext =
        " - Over max storage item size. Please reduce shortcut or autotext length.";
    }
  ...
  function validateShortcuts(completionBlock) {
    ...
    if (duplicates.length) {
      ...
      showModalPopup(
        chrome.i18n.getMessage("ERROR_DUPLICATE_ITEMS") +
          "\n - " +
          duplicates.join("\n - ")
      );
      return false;
    ...
    if (JSON.stringify(data).length >= QUOTA_STORAGE) {
      console.log(chrome.i18n.getMessage("ERROR_OVER_SPACE_QUOTA"));
      showCrouton(
        chrome.i18n.getMessage("ERROR_OVER_SPACE_QUOTA") +
          " Chrome max capacity: " +
          QUOTA_STORAGE +
          " characters",
        "red"
        ...
        chrome.storage.sync.set(shortcuts, function () {
          if (chrome.runtime.lastError) {
            ...
            $("tbody > tr").each(function () {
              ...
              if (
                shortcuts[
                  ATE_CONST.SHORTCUT_PREFIX + $row.find(".shortcut").val()
                ]
              ) {
                $row.addClass("saved");
              }
  ...
  function backupShortcuts() {
    showModalPopup(
      chrome.i18n.getMessage("MESSAGE_BACKUP_WARNING") +
        chrome.i18n.getMessage("PROMPT_CONTINUE"),
      function (response) {
        if (response) {
          saveShortcuts(function () {
            chrome.storage.sync.get(null, function (data) {
              if (chrome.runtime.lastError) {
                ...
                backup[ATE_CONST.APP_BACKUP_TIMESTAMP_KEY] =
                  new Date().getTime();
                chrome.storage.local.set(backup, function () {
                  if (chrome.runtime.lastError) {
                    ...
                    console.error(chrome.runtime.lastError);
                    showCrouton(
                      chrome.i18n.getMessage("ERROR_BACKUP_FAILED"),
                      "red"
                    );
                  } else {
                    // Show success
  ...
  function updateBackupTimestamp() {
    chrome.storage.local.get(
      ATE_CONST.APP_BACKUP_TIMESTAMP_KEY,
      function (data) {
        if (chrome.runtime.lastError) {
          // Check for errors
          console.error(chrome.runtime.lastError);
  ...
  function getEmergencyBackup(completionBlock) {
    chrome.storage.local.get(
      ATE_CONST.APP_EMERGENCY_BACKUP_KEY,
      function (data) {
        if (chrome.runtime.lastError) {
          // Check for errors
          console.error(chrome.runtime.lastError);
  ...
  function promptEmergencyRestore() {
    showModalPopup(
      chrome.i18n.getMessage("MESSAGE_EMERGENCY_RESTORE_WARNING"),
      function (response) {
        if (response) {
          getEmergencyBackup(function (
            data // Restore using emergency backup
          ) {
            console.log(
              "Restoring emergency backup shortcuts: ",
            ...
            chrome.storage.sync.set(
              ...
              function () {
                if (chrome.runtime.lastError) {
                  ...
                  console.error(chrome.runtime.lastError);
                  showCrouton(
                    chrome.i18n.getMessage("ERROR_RESTORE_FAILED"),
                    "red"
                  );
                } // Show success
                else {
  ...
  function restoreShortcuts() {
    ...
    // Confirm restore
    showModalPopup(
      chrome.i18n.getMessage("MESSAGE_RESTORE_WARNING") +
        chrome.i18n.getMessage("PROMPT_CONTINUE"),
      function (response) {
        if (response) {
          getLocalBackup(function (
            data // Restore using backup shortcuts
          ) {
            console.log(
              "Restoring shortcuts: ",
              data[ATE_CONST.APP_BACKUP_KEY]
            );
            chrome.storage.sync.set(
              data[ATE_CONST.APP_BACKUP_KEY],
              function () {
  ...
  function portShortcuts() {
    showPortView(function (newShortcuts) {
      ...
      shortcuts[ATE_CONST.SHORTCUT_VERSION_KEY] =
        metadata[ATE_CONST.SHORTCUT_VERSION_KEY];
    ...
    $("body").append(
      $(document.createElement("div"))
        ...
        .fadeIn(ANIMATION_FAST, function () {
          if (autohide) {
            $(this)
              .delay(TIME_SHOW_CROUTON)
              .fadeOut(ANIMATION_FAST, function () {
                $(this).remove();
              });
          } else {
            // Show a close button
    ...
    $(document.createElement("div"))
      ...
      .append(
        $(document.createElement("h2")).text(
          chrome.i18n.getMessage("TITLE_WARNING_POPUP")
        )
      )
      .append(
        $(document.createElement("p")).html(message.replace(/\n/g, "<br />"))
      )
      .append(
